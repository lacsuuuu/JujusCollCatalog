...
export default function AddPost({ user }) {
  ...
  const handleFiles = (files) => {
    ...
    setPhotos(prev => [...prev, ...Array.from(files).map(file => ({ url: URL.createObjectURL(file), file }))]);
  };
  ...
  const handleDeletePhoto = (e) => {
    ...
    setPhotos(prev => {
      const updated = [...prev];
      updated.splice(currentPhotoIndex, 1);
      return updated;
    });
        ...
        >
          ...
          {photos.length > 0 ? (
            <div style={{ position: 'relative', width: '100%', height: '350px', backgroundColor: '#D4C4C7' }}>
              ...
              {photos.length > 1 && (
                <>
                  <button type="button" className="icon-btn" onClick={handlePrevPhoto} style={{...arrowStyle, left: '10px'}}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
                  </button>
                  <button type="button" className="icon-btn" onClick={handleNextPhoto} style={{...arrowStyle, right: '10px'}}>
